...
try:
    ...
    from rigor_sf.overrides import load_overrides, save_overrides, upsert_edge_override, OverrideEdge
...
if not Path(cfg_path).exists():
    st.sidebar.warning("Config file not found. Create config/config.yaml from config.example.yaml.")
    st.stop()
...
```bash
rigor --config config/config.yaml --sql-dir sql_worksheets/ --phase profile --sample-limit 200000
```
...
4) **Generate ontology**
```bash
rigor --config config/config.yaml --sql-dir sql_worksheets/ --phase generate
```
""")
