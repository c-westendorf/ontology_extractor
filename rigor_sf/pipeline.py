...
def phase_query_gen(cfg, sql_dir: str, run_label=None):
    ...
    log.info("  then run: rigor --config <cfg> --run-dir %s --phase infer", run_dir)
...
def phase_review(cfg):
    ...
    config_path = getattr(cfg, "_config_path", "config/config.yaml")
    try:
        subprocess.run(
...
Offline workflow:
  ...
  rigor --config config/config.yaml --phase review
